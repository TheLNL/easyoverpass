document.addEventListener('DOMContentLoaded', function() {
    ...
    const diffFromDateInput = document.getElementById('diffFromDate');
    const diffToDateInput = document.getElementById('diffToDate');
    const clearDiffDatesBtn = document.getElementById('clearDiffDates');
    ...
    // Get date filter value and format it for Overpass QL
    ...
    // Get diff/adiff settings for Overpass QL
    function getDiffSettings() {
        if (!diffFromDateInput || !diffFromDateInput.value) {
            return null;
        }
        
        const fromDate = new Date(diffFromDateInput.value);
        ...
        if (diffToDateInput && diffToDateInput.value) {
            const toDate = new Date(diffToDateInput.value);
            if (isNaN(toDate.getTime())) {
                return null;
            }
            const toIsoString = toDate.toISOString();
            return `[diff:"${fromIsoString}","${toIsoString}"]`;
        } else {
            // For adiff, only from date is needed (to date defaults to now)
            return `[adiff:"${fromIsoString}"]`;
        }
    }
    ...
    function generateQuery() {
        ...
        // Get diff/adiff settings
        const diffSettings = getDiffSettings();
        
        // Determine output format based on filters
        // Date and diff queries require XML output format
        const hasDateOrDiff = dateFilter || diffSettings;
        const outputFormat = hasDateOrDiff ? 'xml' : 'json';
        ...
        if (queries.length > 0) {
            if (hasAreaInput && !hasBbox) {
                ...
                // Add timeout, date filter, and diff settings at the beginning
        ...
        query += '\n// Print results\n';
        if (outputFormat === 'xml') {
            // For XML output, use standard XML output statements
            query += 'out meta;';
        } else {
...
out body;`;
    ...
    // Update query when date filter changes
    if (dateFilterInput) {
        dateFilterInput.addEventListener('change', generateQuery);
    }
    ...
    // Update query when diff dates change
    if (diffFromDateInput) {
        diffFromDateInput.addEventListener('change', generateQuery);
    }
    if (diffToDateInput) {
        diffToDateInput.addEventListener('change', generateQuery);
    }
    
    // Clear diff dates
    if (clearDiffDatesBtn) {
        clearDiffDatesBtn.addEventListener('click', () => {
            if (diffFromDateInput) diffFromDateInput.value = '';
            if (diffToDateInput) diffToDateInput.value = '';
            generateQuery();
            showToast('Diff dates cleared', 'info');
        });
    }
